...
let rec evaluate_operator current_op =
  function
  | Number a :: Operator op :: Number b :: tl ->
    if op = current_op then
      ...
      [ Number a; Operator op ]
      @ evaluate_operator current_op ([ Number b ] @ tl)
  | r -> r
...
let evaluate_ast tokens =
  let rec aux ast ops =
    ...
    match ast with
    | Group g :: tl -> aux (aux g pemdas @ tl) use_ops
    | Number n :: Operator op :: Group g :: tl ->
      let sub_group = [ Number n; Operator op ] @ aux g pemdas in
      let res = aux sub_group pemdas in
      aux (res @ tl) pemdas
    | ts ->
      (match evaluate_operator (List.head use_ops) ts with
       ...
       | other ->
         print_ast other
         aux other (List.tail use_ops)) in
